...
class CHPP(object):
    ...
    chpp_key              = 'ERhL8EHCzDoHSLIdQBbgaA'
    chpp_secret           = 'KWvWncdQG6wapZFezTcHUWsx8mx8BZoOPEAYjknOiVO'
    ...
    def get_request_token_url(self):
        ...
        with contextlib.closing(urllib.request.urlopen(req.to_url())) as x:
            # send the request
            responseData = x.read()
            request_token = dict(urllib.parse.parse_qsl(responseData.decode('ascii')))
    ...
    def request_resource(self, token, filename, query=[]):
        ...
        with contextlib.closing(urllib.request.urlopen(req.to_url(), timeout=10)) as x:
            ...
            return xmltodict.parse(responseData)['HattrickData']
